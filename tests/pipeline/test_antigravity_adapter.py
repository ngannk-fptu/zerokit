...
from pipeline.services.adapters.antigravity_adapter import AntigravityAdapter, AntigravityNotAvailableError
...
def test_archive_creation(adapter, temp_dirs):
    ...
    # Start call
    import threading
    thread = threading.Thread(target=lambda: adapter.call(prompt, config) if False else None)
    thread.start()
    time.sleep(0.5)
