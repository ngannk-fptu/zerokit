...
class SandboxExecutor:
    ...
    async def _ensure_python_image(self):
        """Ensure the custom zerokit-poc-python image with requests is built."""
        if self._image_ensured:
            return
            
        cmd = ["docker", "images", "-q", "zerokit-poc-python"]
        ...
        if not stdout.strip():
            logger.info("[-] zerokit-poc-python missing. Building Docker image with requests...")
    ...
    async def run_project_poc(self, repo_path: str, script: str, language: str, modified_files: Dict[str, str] = None, timeout: int = None) -> ExecutionResult:
        ...
        images = {
            "PHP": "php:7.4-cli",
            "PYTHON": "zerokit-poc-python",
            "GO": "golang:1.20-alpine",
            "JAVA": "openjdk:11-jre-slim"
        }
        image = images.get(language.upper(), "zerokit-poc-python")
