...
class JoernRunner:
    ...
    async def get_code_graph_structured(self, cpg_path: str) -> Dict:
        """
        Extract nodes (functions, sinks, sources) and edges (calls, data-flow)
        in a structured format for the SemanticGraph.
        """
        query = """
        val nodes = (cpg.method.l.map(m => Map(
            "id" -> m.fullName,
            "type" -> "function",
            "name" -> m.name,
            "file" -> m.filename,
            "line" -> m.lineNumber.getOrElse(0)
        )) ++ cpg.call.nameExact("exec","system","query","execute","eval").l.map(c => Map(
            "id" -> ("sink_" + c.id.toString),
            "type" -> "sink",
            "name" -> c.name,
            "file" -> c.filename,
        ...
        val edges = cpg.call.l.map(c => Map(
            "from" -> c.method.fullName,
            "to" -> (if (c.callee.isEmpty) c.name else c.callee.fullName.head),
            "type" -> "call",
            "line" -> c.lineNumber.getOrElse(0)
        )).toJson

        println("GRAPH_NODES_START")
        println(nodes)
        println("GRAPH_NODES_END")
        println("GRAPH_EDGES_START")
        println(edges)
        println("GRAPH_EDGES_END")
        """
        
        # We use run_script (sync) for now or similar to get the console output
        # For simplicity, let's use a temp script and capture output
        script_file = os.path.join(self.workspace, "graph_ext.sc")
        with open(script_file, "w", encoding="utf-8") as f:
            f.write(query)
            
        cmd = [
            os.path.join(self.joern_home, "joern" + (".bat" if os.name == 'nt' else "")),
            "--script", script_file
        ]
        
        # Revised script that loads the CPG
        query_v2 = f"""
        importCode.bin("{cpg_path.replace('\\', '/')}")
        {query}
        """
        with open(script_file, "w", encoding="utf-8") as f:
            f.write(query_v2)
        ...
        try:
            ...
            # Parse nodes and edges from markers
            nodes = []
            if "GRAPH_NODES_START" in output:
                nodes_raw = output.split("GRAPH_NODES_START")[1].split("GRAPH_NODES_END")[0].strip()
                nodes = json.loads(nodes_raw)
            
            edges = []
            if "GRAPH_EDGES_START" in output:
                edges_raw = output.split("GRAPH_EDGES_START")[1].split("GRAPH_EDGES_END")[0].strip()
                edges = json.loads(edges_raw)
                
            return {"nodes": nodes, "edges": edges}
        except Exception as e:
            logger.error(f"Structured graph extraction failed: {e}")
