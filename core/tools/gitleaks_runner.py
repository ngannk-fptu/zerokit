...
class GitleaksRunner:
    ...
    async def scan_repo_with_evidence(self, repo_path: str, output_format: str = "json"):
        """
        Evidence-aware scan for PipelineObserver integration.

        Returns:
            Tuple: (findings: List[Dict], exit_code: int, stdout: str, stderr: str, command: str)

        Exit code semantics for PipelineObserver gate:
            0  → clean (no secrets) — accepted, 0 findings
            1  → secrets found — accepted, N findings  ← BOTH 0 and 1 are "tool ran OK"
            2+ → tool error — rejected by gate
        """
        if not self.available:
            return [], 2, "", "Gitleaks binary not found", "gitleaks [not found]"
        ...
        try:
            ...
            # Gitleaks: exit 0 = clean, exit 1 = found secrets — both mean the tool ran OK
            # Normalize: treat 0 and 1 as "success" for the evidence gate
            gate_exit = 0 if exit_code in (0, 1) else exit_code
            ...
            logger.info(f"[Evidence] gitleaks exit={exit_code} (gate={gate_exit}), {len(findings)} findings")
            return findings, gate_exit, stdout, stderr, cmd_str

        except subprocess.TimeoutExpired:
            return [], 2, "", "Gitleaks timeout after 120s", cmd_str
        except Exception as e:
            return [], 2, "", str(e), cmd_str
