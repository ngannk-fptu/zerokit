...
from ..tools.joern_queries import QUERY_TEMPLATES, MEMORY_SAFETY_QUERIES
...
class Detector:
    def __init__(self, semgrep_runner: SemgrepRunner = None, joern_runner: JoernRunner = None):
        ...
        self.codeql_runner = CodeQLRunner(codeql_path=config.CODEQL_BIN) # NEW
        self.joern_runner = joern_runner or JoernRunner() # NEW: Joern Integration
        ...
        while attempt < MAX_RETRIES:
            ...
            if self.state_manager.is_rule_blacklisted(query_content):
                ...
                query_content = self.llm_gateway.fix_semgrep_rule(
                    query_content, 
                    f"PREVIOUS_FAILED_ATTEMPT: {last_err}. DO NOT use this pattern again."
                )
            ...
            try:
                ...
                if result.success:
                    ...
                    if "syntax" in result.error_msg.lower() or "yaml" in result.error_msg.lower():
                        ...
                        fixed_resp = self.llm_gateway.fix_semgrep_rule(query_content, result.error_msg)
                        ...
                        if "rule_content" in fixed_resp:
                             try:
                                 ...
                                 data = json.loads(clean_fix.strip())
                                 query_content = data.get("rule_content", "")
    ...
    def _normalize_finding(self, res: dict, tool_name: str) -> Optional[StaticFinding]:
        ...
        if tool_name == "semgrep":
            ...
            tag = metadata.get("tag") # 'surface' or None
            
            # If it's a surface finding, we might want to handle it differently
            # For now, map it to a Finding but we will filter it in the scan loop
            ...
            return StaticFinding(
                ...
                metadata={"tag": tag, "type": metadata.get("type")} # Store tag
            )
        return None
