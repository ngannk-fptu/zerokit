...
class Orchestrator:
    ...
    def _build_generic_hypotheses(self) -> List[Hypothesis]:
        ...
        return hypotheses
            ...
            # Upgrade 5.2: Taint Path Explanation
            from .agents.explainer import Explainer
            self.explainer = Explainer(llm_gateway=self.llm_gateway)

            # Upgrade 2: Inline DV Loop
            high_signal = []
            for f in self.context.static_findings:
                if f.severity in (FindingSeverity.CRITICAL, FindingSeverity.HIGH) and self.context.code_graph:
                    try:
                        explanation = await self.explainer.explain_finding(f, self.context.code_graph)
                        f.metadata["taint_explanation"] = explanation
                        logger.info(f"  [Explainer] Path trace generated for {f.id}")
                    except Exception as e:
                        logger.error(f"  [Explainer] Failed to explain finding {f.id}: {e}")

                if f.directed_fuzz_candidate:
                    high_signal.append(f)

            if high_signal:
                logger.info(f"  [DV-LOOP] {len(high_signal)} high-signal candidates identified. Triggering inline verification...")
                asyncio.run(self.run_stage_verification(findings_to_verify=high_signal))
